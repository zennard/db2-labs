...
def generate_random_message():
    return fake.sentence(nb_words=10, variable_nb_words=True, ext_word_list=None) if random.choice(
        [True, False]) else 'spam'
...
class User(Thread):
    def __init__(self, connection, username, users_list, users_count):
        ...
        user_demo.register(conn, username)
        self.user_id = user_demo.sign_in(conn, username)
    ...
    def run(self):
        for x in range(6):
            ...
            user_demo.create_message(self.connection, message_text, self.user_id, receiver)
