...
def create_message(conn, message_text, sender_id, consumer) -> int:
    ...
    pipeline.zincrby("sent:", 1, "user:%s" % get_username(conn, sender_id))
...
def get_username(conn, user_id):
    return conn.hmget("user:%s" % user_id, ["login"])[0]
...
def print_messages(connection, user_id):
    ...
    for message_id in messages:
        ...
        sender_id = message[0]
        print("User with username: '%s' sent message with content: '%s'" % (get_username(connection, sender_id), message[1]))
